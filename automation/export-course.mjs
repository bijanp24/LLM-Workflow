// Reusable deploy-time exporter. Copy this file to `.github/scripts/export-course.mjs`
// in any applied repo, alongside `export-course-on-deploy.yml`. On deploy it sends
// a `generate-course` repository_dispatch to the LLM Academy content repo, which
// drafts a course from the seed material and opens a PR for review.
//
// Seed precedence:
//   1. docs/lessons/next-course.md  (author-controlled — you write the topic/outline)
//   2. otherwise, the latest commit subject/body + changed file list (diff fallback)
//
// Env:
...
//   GITHUB_REPOSITORY / GITHUB_REF_NAME / GITHUB_SHA — provided by Actions.
